...
import "draw-and-guess-server/internal/graph/model"
...
type RoundStartData struct {
	...
	Drawer    string `json:"drawer" example:"user-001" description:"그림 그릴 플레이어"`
	Topic     string `json:"topic,omitempty" example:"사과" description:"주제 (그림 그리는 사람만 받음)"`
	TopicHint string `json:"topic_hint" example:"과일 (2자)" description:"주제 힌트"`
	TimeLimit int    `json:"time_limit" example:"60" description:"제한 시간 (초)"`
}
...
type GameEndData struct {
	...
	FinalScore []*model.User `json:"final_score" description:"최종 점수판"`
}
