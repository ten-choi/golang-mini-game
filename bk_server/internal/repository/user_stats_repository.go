...
// UserStatsRepository defines the interface for user stats data access
...
func NewUserStatsRepository(collection *mongo.Collection) UserStatsRepository {
	return &userStatsRepository{collection: collection}
}
...
func (r *userStatsRepository) GetByUserID(ctx context.Context, userID string) (*models.UserStats, error) {
	...
	if err == mongo.ErrNoDocuments {
		return nil, common.NewNotFoundError("user stats not found")
	}
	if err != nil {
		return nil, common.NewInternalError("failed to query user stats", err)
	}

	return &stats, nil
}

func (r *userStatsRepository) GetLeaderboard(ctx context.Context, gameType string, limit int) ([]*models.UserStats, error) {
...
func (r *userStatsRepository) UpsertStats(ctx context.Context, stats *models.UserStats) error {
	filter := bson.M{
	...
	if err != nil {
		return common.NewInternalError("failed to upsert user stats", err)
	}
