...
type Quiz struct {
	...
	Hint        string      `db:"hint" json:"hint"`               // 힌트 (선택사항)
	Explanation string      `db:"explanation" json:"explanation"` // 정답 설명 (선택사항)
	ImageURL    string      `db:"image_url" json:"image_url"`     // 이미지 URL (Guess 타입용)
	UsageCount  int         `db:"usage_count" json:"usage_count"` // 사용 횟수
	IsActive    bool        `db:"is_active" json:"is_active"`     // 활성화 여부
...
type GuessQuiz struct {
	...
	ImageURL     string            `db:"image_url" json:"image_url"`       // 참고 이미지 URL
	UsageCount   int               `db:"usage_count" json:"usage_count"`
	IsActive     bool              `db:"is_active" json:"is_active"`
	CreatedAt    time.Time         `db:"created_at" json:"created_at"`
	UpdatedAt    time.Time         `db:"updated_at" json:"updated_at"`
